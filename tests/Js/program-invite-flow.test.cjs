...
function setup(count, responses) {
 const element = () => ({style:{},dataset:{},value:'',hidden:false,disabled:false,textContent:'',addEventListener(){},click(){},focus(){this.focused=true},replaceChildren(){this.items=[]},appendChild(item){(this.items??=[]).push(item)}});
 const ids = Object.fromEntries(['program-invite','invite-rows','invite-progress','invite-tools','invite-optional','invite-group','invite-send','invite-add','invite-import','invite-file','invite-close','invite-cancel','invite-done','invite-finish','program-invite-form','invite-success','invite-title','invite-success-title','invite-receipts','invite-phone','invite-territory','invite-search','invite-download'].map(id=>[id,element()]));
 ...
 let download;class DownloadURL extends URL { static createObjectURL(blob){download=blob;return 'blob:test';} static revokeObjectURL(){} }
 vm.runInNewContext(source,{URL:DownloadURL,Blob,document:{getElementById:id=>ids[id],createElement:element,querySelector:()=>({value:'csrf'})},location,fetch:async(url,options)=>{calls.push(JSON.parse(options.body));const ok=responses.shift();return {ok,json:async()=>ok?{}:{message:'Please try again'}}},setTimeout:fn=>fn()});
 return {ids,rows,calls,location,getDownload:()=>download,submit:()=>ids['program-invite-form'].onsubmit({preventDefault(){}})};
}
...
test('bulk search filters visible rows without dropping recipients',()=>{
 const s=setup(2,[]);s.ids['invite-search'].value='test1';s.ids['invite-search'].oninput();assert.equal(s.rows[0].style.display,'none');assert.equal(s.rows[1].style.display,'');s.ids['invite-send'].onclick();assert.equal(s.rows[0].style.display,'');
});
test('failed CSV exports only failures and escapes spreadsheet formulas',async()=>{
 const s=setup(2,[true,false]);s.rows[1].querySelector('[data-name]').value='=HYPERLINK("bad")';await s.submit();assert.equal(s.ids['invite-download'].hidden,false);s.ids['invite-download'].onclick();const csv=await s.getDownload().text();assert.ok(csv.includes('test1@example.com'));assert.ok(!csv.includes('test0@example.com'));assert.ok(csv.includes("'=HYPERLINK"));assert.ok(csv.includes('""bad""'));
});
