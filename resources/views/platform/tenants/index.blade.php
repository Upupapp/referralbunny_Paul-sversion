...
<div class="space-y-5">
    ...
    {{-- Filters --}}
    <div class="card">
        <form method="GET" class="flex flex-col sm:flex-row gap-3 flex-wrap items-center">
            ...
            <select name="status" class="form-input sm:w-36">
                <option value="">All Status</option>
                <option value="active"   {{ request('status') === 'active'   ? 'selected' : '' }}>Active</option>
                <option value="trial"    {{ request('status') === 'trial'    ? 'selected' : '' }}>Trial</option>
                <option value="inactive" {{ request('status') === 'inactive' ? 'selected' : '' }}>Inactive</option>
            </select>
            <select name="health_level" class="form-input sm:w-44">
                <option value="">All Health</option>
                <option value="healthy"         {{ request('health_level') === 'healthy'         ? 'selected' : '' }}>Healthy (80+)</option>
                <option value="needs_attention" {{ request('health_level') === 'needs_attention' ? 'selected' : '' }}>Needs Attention (50–79)</option>
                <option value="at_risk"         {{ request('health_level') === 'at_risk'         ? 'selected' : '' }}>At Risk (&lt;50)</option>
            </select>
            <select name="industry" class="form-input sm:w-48">
            ...
            @if(request()->hasAny(['search','status','industry','health_level']))
                <a href="{{ route('platform.tenants') }}" class="btn-secondary shrink-0">Clear</a>
            @endif
        </form>
    </div>
