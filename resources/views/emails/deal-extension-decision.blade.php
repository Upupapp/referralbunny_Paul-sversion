...
@section('content')
@php
$dealUrl = url('/reseller/' . $deal->tenant_id . '/deals/' . $deal->id);
@endphp

@if($decision === 'approved')
<p class="greeting">Your extension request was approved!</p>
<p class="text">Hi <strong>{{ $resellerName }}</strong>, great news — your deadline extension request for <strong>{{ $deal->name }}</strong> has been approved.</p>
<div class="highlight-box highlight-teal">
    <div class="highlight-title" style="color:#0D9488">Extension Details</div>
    <div class="highlight-text">
        <strong>Deal:</strong> {{ $deal->name }}<br>
...
<p class="text">Hi <strong>{{ $resellerName }}</strong>, unfortunately your extension request for <strong>{{ $deal->name }}</strong> has been rejected.</p>
...
<p class="text">Hi <strong>{{ $resellerName }}</strong>, the admin has requested clarification on your extension request for <strong>{{ $deal->name }}</strong>.</p>
@if($extensionRequest->admin_note)
