...
<div class="flex min-h-screen">
    ...
    <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'"
        ...
        @php
            if (auth('tenant')->check()) {
                ...
                $profileName     = $saUser ? \App\Services\UserDisplayNameService::resolve($saUser, false, 'super_admin') : 'Super Admin';
                $profileEmail    = $saUser?->email ?? '';
                $profilePhotoUrl = $saUser ? \App\Services\UserDisplayNameService::photoUrl($saUser) : null;
                $initials        = $saUser ? \App\Services\UserDisplayNameService::initials($saUser) : 'SA';
    ...
    <div class="flex-1 flex flex-col min-w-0">
            ...
            <div class="flex items-center gap-1 ml-auto shrink-0">
                ...
                <div x-data="{ open: false }" class="relative ml-1">
                    ...
                    <div x-show="open" @click.outside="open = false" x-cloak
                        ...
                        @php
                            $profileRoute = '#';
                            if ($isSuperAdmin)    $profileRoute = route('platform.profile');
                            elseif ($isTenantAdmin && isset($tenant)) $profileRoute = route('tenant.profile', $tenant->id);
                        @endphp
                        <div class="py-1 border-b border-gray-100">
                            <a href="{{ $profileRoute }}" @click="open = false"
        ...
        @endif
        @if(session('info'))
            <div role="alert" class="mx-4 lg:mx-6 mt-4 p-3 bg-blue-50 border border-blue-200 rounded-xl text-blue-700 text-sm flex items-center gap-2">
