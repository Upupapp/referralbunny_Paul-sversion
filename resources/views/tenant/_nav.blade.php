...
if ($isAdminMgr) {
    try {
        $criticalBadge =
            (int) \Illuminate\Support\Facades\DB::table('leads')
                ->where('tenant_id', $tenantId)
                ->where('status', 'expiring')
                ->count()
            + (int) \Illuminate\Support\Facades\DB::table('deal_assignment_extension_requests')
                ->where('tenant_id', $tenantId)
                ->whereIn('status', ['pending_review', 'clarification_requested'])
                ->count()
            + (int) \Illuminate\Support\Facades\DB::table('deal_approval_requests')
                ->where('tenant_id', $tenantId)
