...
<div x-data="{
    ...
    <div>
        ...
        <div id="nav-admin"
            ...
            <a href="{{ route('tenant.settings.branding.index', $tenantId) }}"
               aria-current="{{ request()->routeIs('tenant.settings.branding.*') ? 'page' : 'false' }}"
               @click="window.dispatchEvent(new CustomEvent('sidebar-close'))"
               class="nav-child {{ request()->routeIs('tenant.settings.branding.*') ? 'nav-child-active' : '' }}">
                <svg class="nav-icon-sm" fill="none" stroke="currentColor" viewBox="0 0 24 24">
