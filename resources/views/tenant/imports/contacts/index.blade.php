...
@section('topbar-actions')
    <a href="{{ route('tenant.imports.contacts.template', $tenant->id) }}" class="btn-secondary">
        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
        </svg>
        <span class="hidden sm:inline">Download Template</span>
    </a>
    <button type="button" x-data @click="$dispatch('open-contact-import')" class="btn-primary">
        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
@section('content')
<div x-data="{ showUpload: false }" @open-contact-import.window="showUpload = true" class="space-y-5">
    ...
    <div class="card">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div>
                ...
                <p class="text-gray-400 text-sm mt-0.5">Import contacts with deal, organization, and invite staging.</p>
            </div>
    ...
    @php
        ...
        $needsReview    = $batches->getCollection()->filter(fn($b) => in_array($b->status, ['needs_review', 'completed_with_warnings']))->count();
    @endphp
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div class="kpi-card">
            <div>
                <p class="text-xs font-medium text-gray-500 mb-1">Contacts Created</p>
                <p class="text-2xl font-bold text-[#1E1B4B] tabular-nums">{{ number_format($totalCreated) }}</p>
                <p class="text-xs text-gray-400 mt-1">Contacts successfully imported</p>
            </div>
            <div class="kpi-icon" style="background: #EDE9FE;">
                <svg class="w-5 h-5" style="color: #7B61FF;" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="kpi-card">
            <div>
                <p class="text-xs font-medium text-gray-500 mb-1">Needs Review</p>
                <p class="text-2xl font-bold tabular-nums {{ $needsReview > 0 ? 'text-amber-600' : 'text-[#1E1B4B]' }}">{{ number_format($needsReview) }}</p>
                <p class="text-xs text-gray-400 mt-1">Batches with warnings or pending review</p>
            </div>
            <div class="kpi-icon {{ $needsReview > 0 ? 'bg-amber-50' : 'bg-gray-50' }}">
                <svg class="w-5 h-5 {{ $needsReview > 0 ? 'text-amber-500' : 'text-gray-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/>
                </svg>
            </div>
        </div>
    ...
    <div class="card">
                    ...
                    <tr class="table-head">
                        ...
                        <th>
                            <span class="inline-flex items-center gap-1">
                                <svg class="w-3.5 h-3.5" style="color: #7B61FF;" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <tbody>
                    ...
                    <tr class="table-row">
                        ...
                        <td class="tabular-nums text-gray-500">
                            {{ number_format($batch->duplicate_rows ?? 0) }}
                        </td>
                        <td>
                            @php $sameEmailCount = $batch->same_email_different_referrer_rows ?? 0; @endphp
                            @if($sameEmailCount > 0)
                            <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold" style="background: #EDE9FE; color: #7B61FF;">
                                <svg class="w-3 h-3 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
                                </svg>
                                {{ number_format($sameEmailCount) }}
                            </span>
                            @else
        ...
        <div class="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p class="text-gray-400 text-sm mt-1 max-w-xs">Upload your first contacts file to get started.</p>
        ...
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden"
            ...
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
                <div class="flex items-center gap-2.5">
                    ...
                    <div>
                        <h2 class="text-[#1E1B4B] font-semibold text-base">Upload Contact Import File</h2>
                        <p class="text-xs text-gray-400">Contacts template — .csv or .xlsx</p>
                    </div>
                </div>
            ...
            {{-- Modal body --}}
            <form action="{{ route('tenant.imports.contacts.upload', $tenant->id) }}"
                  method="POST"
                ...
                <div class="p-6 space-y-4">
                    ...
                    {{-- Helper text --}}
                    <div class="flex items-start gap-2 p-3 rounded-xl" style="background: #EDE9FE;">
                        <svg class="w-4 h-4 shrink-0 mt-0.5" style="color: #7B61FF;" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
                        </svg>
                        <p class="text-xs" style="color: #5B45DF;">
                            At least one of <span class="font-semibold">Email</span> or <span class="font-semibold">Phone</span> is required per contact row.
                        </p>
                    </div>
