@extends('layouts.app')
@section('title', 'Import Preview — Deal Import')
...
@php
$isAdmin   = auth('tenant')->check() || auth('web')->check();
$isLguIds  = $tenant->id === 'lgu-ids';
$unmapped  = $batch->unmapped_columns_json ?? [];
$hasUnmapped = !empty($unmapped);
$initiateUrl = route('tenant.imports.deals.initiate-adoption', [$tenant->id, $batch->id]);
$verifyUrl   = route('tenant.imports.deals.verify-adoption',   [$tenant->id, $batch->id]);
@endphp

@section('content')
<div x-data="dealImportPreview()" x-init="init()" class="space-y-5">
    ...
    <div class="sticky top-0 z-20 card py-3 shadow-md border-b border-gray-100">
        <div class="flex flex-col sm:flex-row sm:items-center gap-3">
            ...
            <div class="flex items-center gap-2 shrink-0">
                ...
                <form action="{{ route('tenant.imports.deals.execute', [$tenant->id, $batch->id]) }}" method="POST" id="confirm-import-form">
                    @csrf
                    <button type="submit"
        ...
        <div class="flex flex-col sm:flex-row sm:items-start gap-3 mb-4">
            ...
            <div class="w-10 h-10 rounded-xl flex items-center justify-center shrink-0" style="background: #EDE9FE;">
                <svg class="w-6 h-6" viewBox="0 0 32 32" fill="none" xmlns="http://www.w3.org/2000/svg">
                    {{-- Simplified R Bunny "helper" inline SVG: bunny head with ears + question hint --}}
                ...
                <tbody>
                    ...
                    @php
                        ...
                        $rowBorder = match($statusKey) {
                            ...
                            'unknown_referrer'=> 'border-l-2 border-l-yellow-400',
                    ...
                    <tr class="table-row {{ $rowBorder }} transition-all"
                        ...
                        {{-- Referrer Email --}}
                        <td>
                            <span class="text-sm {{ $statusKey === 'unknown_referrer' ? 'text-yellow-700 font-medium' : 'text-gray-700' }}">
                        ...
                        {{-- Stage --}}
                        <td>
                            <span class="text-xs text-gray-600">{{ $row->raw_data['deal_stage'] ?? $row->raw_data['stage'] ?? '—' }}</span>
                        </td>
                        ...
                        <td colspan="10" class="px-8 py-3">
                            ...
                            <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs {{ ($existing['stage'] ?? '') !== ($row->raw_data['deal_stage'] ?? $row->raw_data['stage'] ?? '') ? 'text-amber-700 font-semibold' : 'text-gray-700' }} mt-0.5">
                                        {{ $existing['stage'] ?? '—' }}
                                    </p>
                                </div>
                                <div>
                    ...
                    @empty
                    <tr>
                        <td colspan="10" class="py-12 text-center text-gray-400 text-sm">No rows found.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <div class="card">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div class="flex flex-col sm:flex-row gap-2 shrink-0">
                ...
                <form action="{{ route('tenant.imports.deals.execute', [$tenant->id, $batch->id]) }}" method="POST">
                    @csrf
...
@push('scripts')
<script>
const saveUrl     = '{{ route('tenant.imports.deals.column-actions', [$tenant->id, $batch->id]) }}';
const initiateUrl = '{{ $initiateUrl }}';
    ...
    return {
        ...
        async initiateAndVerify() {
            ...
            try {
                ...
                // Step 1: initiate
                const r1 = await fetch(initiateUrl, {
                    method:  'POST',
                    headers: {
                        'Content-Type':  'application/json',
                        'X-CSRF-TOKEN':  csrf,
...
// ── Main deal import preview component ───────────────────────
