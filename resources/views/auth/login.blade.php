...
<head>
    ...
    <title>Log In — ReferralBunny.ai</title>
...
<div class="login-page">
    <div class="login-card">
        ...
        <div class="card-left">
            ...
            <p style="margin:0 0 2rem;font-size:.6875rem;font-weight:600;color:#9ca3af;letter-spacing:.1em;text-transform:uppercase">Welcome back</p>
            ...
            @elseif (session('success'))
                <div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:12px;padding:.75rem 1rem;margin-bottom:1.25rem;font-size:.875rem;color:#166534;display:flex;align-items:center;gap:.5rem">
                    <svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24" style="flex-shrink:0"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
                    {{ session('success') }}
                </div>
            @elseif (session('status'))
                <div style="background:#eff6ff;border:1px solid #bfdbfe;border-radius:12px;padding:.75rem 1rem;margin-bottom:1.25rem;font-size:.875rem;color:#1d4ed8;display:flex;align-items:center;gap:.5rem">
            ...
            <form method="POST" action="{{ route('login') }}" class="form-wrap">
                ...
                <div style="margin-bottom:.25rem">
                    ...
                    <div class="pw-wrap" x-data="{ show: false }">
                        <input id="password" name="password" type="password" :type="show ? 'text' : 'password'"
                               autocomplete="current-password" required
                               class="field-input" style="padding-right:2.75rem"
                               placeholder="••••••••">
                        <button type="button" class="pw-toggle" @click="show = !show" :aria-label="show ? 'Hide password' : 'Show password'">
                ...
                <div style="display:flex;align-items:flex-start;gap:.625rem;margin:.875rem 0 1rem">
                    ...
                    <div>
                        ...
                        <p id="remember_sa_hint" style="margin:.125rem 0 0;font-size:.6875rem;color:#9CA3AF;line-height:1.4">
                            Only keep yourself signed in on trusted devices.
                        </p>
                    </div>
                </div>
            ...
            <p style="margin-top:1.25rem;font-size:.875rem;color:#6b7280">New company? <a href="{{ route('tenant.create') }}" style="color:#7B61FF">Create an account</a></p>
        ...
        <div class="card-right">
            ...
            <div style="position:relative;z-index:1">
                ...
                <h2 style="margin:0 0 .5rem;font-size:1.125rem;font-weight:700;color:#fff;line-height:1.4">Your referrals.<br>One place to sign in.</h2>
                <p style="margin:0 auto;font-size:.75rem;color:rgba(255,255,255,.5);line-height:1.6;max-width:180px">Manage your program, track referrals, and grow together.</p>
            </div>
        </div>
    </div>
</div>
