...
<head>
    ...
    <script>if(!sessionStorage.getItem('rb_intro_setup')){document.documentElement.classList.add('show-intro');}</script>
    <style>
...
<div id="intro">
    ...
    <div style="position:relative;z-index:1;display:flex;flex-direction:column;align-items:center;gap:.25rem;text-align:center">
        <div class="at" style="padding:0 1rem">
            <h1 style="margin:0;font-size:1.5rem;font-weight:700;color:#ffffff;letter-spacing:-.02em;text-shadow:0 1px 8px rgba(0,0,0,.3)">You've been invited!</h1>
            <p style="margin:.375rem 0 0;font-size:.875rem;color:rgba(255,255,255,.7)">Set up your account and start claiming deals.</p>
        </div>
...
<script>
    ...
    if (document.documentElement.classList.contains('show-intro')) {
        sessionStorage.setItem('rb_intro_setup', '1');
