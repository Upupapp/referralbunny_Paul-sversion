...
<div class="max-w-3xl mx-auto space-y-4"
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden divide-y divide-gray-50">
        ...
        <div class="px-4 py-4 sm:px-5"
            ...
            {{-- Description (if any) --}}
            @if($task->description)
            <p class="text-xs text-gray-500 mb-2 ml-4 leading-relaxed line-clamp-2">{!! rb_linkify($task->description) !!}</p>
            @endif
            ...
            <div class="flex flex-wrap items-center gap-3 text-[11px] text-gray-400 ml-4 mb-3">
                ...
                @endif
                @php
                $dealLinkId = $task->taskable_type === 'lead' ? ($task->taskable_id ?? ($task->metadata['deal_id'] ?? null)) : ($task->metadata['deal_id'] ?? null);
                @endphp
                @if($dealLinkId)
                <a href="/reseller/{{ $tenant->id }}/deals/{{ $dealLinkId }}"
                   class="inline-flex items-center gap-1 font-semibold no-underline"
                   style="color:#7B61FF">
