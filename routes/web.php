...
Route::post('/r/{code}/click', \App\Http\Controllers\Web\ReferralShortLinkController::class)
    ->where('code', '[a-z0-9]{8}')->middleware('throttle:120,1')->name('referral.click');
...
Route::middleware(['auth:tenant,web', 'tenant.access', 'legal.agreements'])->prefix('tenant/{tenantId}')->name('tenant.')->group(function () {
    ...
    Route::prefix('programs')->name('programs.')->group(function () {
        ...
        Route::post('/{programId}/members/{membershipId}/renew', [\App\Http\Controllers\Web\ProgramReferrersController::class, 'renew'])->middleware('throttle:5,1')->name('members.renew');
        Route::post('/{programId}/members/{membershipId}/resend', [\App\Http\Controllers\Web\ProgramReferrersController::class, 'resend'])->middleware('throttle:5,1')->name('members.resend');
        ...
        // Members tab
        Route::post('/{programId}/members/invite', [\App\Http\Controllers\Web\ProgramMembershipController::class, 'inviteReferrer'])->middleware('throttle:60,1')->name('members.invite');
