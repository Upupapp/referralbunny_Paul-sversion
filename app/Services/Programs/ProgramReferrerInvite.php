...
class ProgramReferrerInvite {
 public function send(Program $program,string $name,string $email,array $details=[],bool $renewExpired=false): Reseller {
  ...
  $reseller=DB::transaction(function()use($program,$name,$email,$details,$renewExpired){
   ...
   if($renewExpired && !$r->password && $r->setupExpiresAt()?->isPast())$r->update(['setup_token'=>Str::random(64),'setup_token_created_at'=>now()]);
   if(!$r->password && (!$r->setup_token || $r->setupExpiresAt()?->isPast()))throw ValidationException::withMessages(['email'=>'This account needs its setup invitation renewed before it can join.']);
