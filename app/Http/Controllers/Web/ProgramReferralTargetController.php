...
class ProgramReferralTargetController extends Controller {
 public function store(Request $request, string $tenantId, string $programId) {
  ...
  $campaign=app(\App\Services\Programs\CampaignPeriod::class)->forProgram($program);
  if ($campaign) $request->merge(['starts_on'=>$campaign['starts_on'],'ends_on'=>$campaign['ends_on']]);
